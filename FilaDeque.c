...
int addLast(tdeque *minhaDeque, tdado novoDado){
	...
	novoNo->prox = NULL;
	novoNo->ant = minhaDeque->fim; // alteração deque 
...
void printInvertida(tdeque minhaDeque){ // alterar
	while(minhaDeque.fim != NULL){
		printf("<- %x| %d [%x] | %x ->\n",
		minhaDeque.fim->ant, minhaDeque.fim->dado, minhaDeque.fim, minhaDeque.fim->prox);
		minhaDeque.fim = minhaDeque.fim->ant; // deslocando para o proximo 
